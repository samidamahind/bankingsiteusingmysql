...
<head>
    ...
    <title>Samida Mahind | TSF e-Banking Services</title>
</head>
    ...
    <div class="container">
        <div class="left_container">
            ...
            <div id="footer">
                ...
                <span>A Project By <strong title="Samida Popat Mahind" style="font-family:'Dancing Script';font-size: 1.4rem;font-weight: 700;cursor:pointer;">Samida Mahind</strong></span></br>
...
<script type="text/javascript">
    ...
    function WDvisible(){
        ...
        $getRName="SELECT * FROM tsf2 WHERE Customer_ID='$RecieverID'";
        $Reciever=mysqli_fetch_array(mysqli_query($conn,$getRName));
        $RBal=$Reciever['Current_Balance'];

        //Sender's DATA fetching
        $getRName="SELECT * FROM tsf2 WHERE Customer_ID='$SenderId'";
...
$amountupdate1="UPDATE `tsf2` SET `Current_Balance` = '$sent' WHERE `tsf2`.`Customer_ID` = '$SenderId'";
        $r1=mysqli_query($conn,$amountupdate1);

        //To Transaction Table
        $transfer = "INSERT INTO tsf1(Sender_ID, Sender_Name, Receiver_ID, Receiver_Name, Trans_Amount) VALUES ('$SenderId','$SenderName','$RecieverID','$Reciever[Customer_Name]','$amount')";
                $r2 = mysqli_query($conn,$transfer);

        //Reciever's DATA fetching
                $Recieve= (int)$RBal + $_POST['amount'];
        $amountupdate2="UPDATE `tsf2` SET `Current_Balance` = '$Recieve' WHERE `tsf2`.`Customer_ID` = '$RecieverID'";
